...
| Passkeys/WebAuthn are not implemented as browser-facing support | Backend-only C# prototype covers RP ID validation, protected private-key storage, KeePass approval grant/deny handling, assertion signing, sign-count increment, required/unknown user-verification rejection, platform authenticator-attachment rejection, unsupported requested WebAuthn extension rejection, disabled protocol permission gates, and feature-gated trusted-browser passkey permission controls; extension manifests still do not request WebAuthn proxy permissions | Use `docs/passkeys-webauthn-design.md` before adding browser proxy integration, enabled passkey protocol behavior, or store-listing claims. |
