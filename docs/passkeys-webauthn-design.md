...
- Assertion signature verification against generated public keys, with exact authenticatorData length and user-present-only flags, `clientDataJSON` origin, assertion credential ID, user handle, sign-count metadata, and stored public-key COSE metadata required to match the expected EC2/P-256/ES256 credential. Covered by backend tests.
