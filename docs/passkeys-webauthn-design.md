...
- Reject create requests whose `pubKeyCredParams` do not allow ES256 (`alg: -7`) public-key credentials while the prototype only generates ES256 keys.
...
- Browser-proxy create algorithm gating for ES256 (`alg: -7`) public-key credentials. Covered by non-packaged proxy JS tests before bridge dispatch.
