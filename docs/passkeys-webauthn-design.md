...
- Clear pending WebAuthn sessions on browser cancellation, browser revoke, timeout, or KeePass database close. Backend and bridge tests cover timeout, client clear, explicit `passkeys.cancel`, trusted-client revoke cleanup, and the handler cleanup path used by KeePass `FileClosingPre`/`FileClosed` events. Browser lock cleanup remains future UI integration work.
