...
- Reject create/get complete responses, including object payloads and pre-serialized response JSON, that omit required WebAuthn response fields, carry invalid base64url WebAuthn fields, carry mismatched serialized credential ID fields, or use a serialized credential type other than exact `public-key` before completing the browser WebAuthn request.
