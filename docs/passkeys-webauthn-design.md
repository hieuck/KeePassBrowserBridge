...
- User-verification, resident-key, and transport metadata normalization plus KeePass storage round-trip, with `userVerification=required` rejected until a real KeePass-side verification step exists. Covered by backend tests and non-packaged proxy JS tests.
