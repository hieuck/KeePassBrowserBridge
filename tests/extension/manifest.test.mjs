...
const __filename = (() => {
  ...
  catch { return path.join(process.cwd(), 'tests', 'extension', 'manifest.test.mjs'); }
})();
...
const assemblyInfo = fs.readFileSync(path.join(projectRoot, 'src', 'Properties', 'AssemblyInfo.cs'), 'utf8');
const pluginProject = fs.readFileSync(path.join(projectRoot, 'src', 'KeePassBrowserBridge.csproj'), 'utf8');
const bridgeTestProject = fs.readFileSync(path.join(projectRoot, 'tests', 'KeePassBrowserBridge.Tests.csproj'), 'utf8');
const bridgeSettings = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'BridgeSettings.cs'), 'utf8');
const bridgeMethodPolicy = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'BridgeMethodPolicy.cs'), 'utf8');
const protocolModels = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'ProtocolModels.cs'), 'utf8');
const bridgeRequestHandler = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'BridgeRequestHandler.cs'), 'utf8');
const trustedClientStore = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'TrustedClientStore.cs'), 'utf8');
const updateCheckerSource = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'UpdateChecker.cs'), 'utf8');
const passkeyServiceSource = fs.readFileSync(path.join(projectRoot, 'src', 'Bridge', 'PasskeyService.cs'), 'utf8');
...
const artifactVerifyScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-release-artifacts.ps1'), 'utf8');
const verifierScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify.ps1'), 'utf8');
const signedReleaseSmokeScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-signed-release-smoke.ps1'), 'utf8');
const cleanSourceSmokeScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-clean-source-smoke.ps1'), 'utf8');
const realSiteMatrixScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-real-site-matrix.mjs'), 'utf8');
const securityThreatModelScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-security-threat-model.mjs'), 'utf8');
const storeScreenshotsVerifyScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'verify-store-screenshots.mjs'), 'utf8');
const screenshotScript = fs.readFileSync(path.join(projectRoot, 'scripts', 'capture-store-screenshots.mjs'), 'utf8');
const screenshotWrapper = fs.readFileSync(path.join(projectRoot, 'scripts', 'capture-store-screenshots.ps1'), 'utf8');
...
const extensionReadme = fs.readFileSync(path.join(projectRoot, 'extension', 'README.md'), 'utf8');
const extensionTestingGuide = fs.readFileSync(path.join(projectRoot, 'extension', 'TESTING_GUIDE.md'), 'utf8');
const realSiteValidation = fs.readFileSync(path.join(projectRoot, 'docs', 'real-site-validation.md'), 'utf8');
const architecture = fs.readFileSync(path.join(projectRoot, 'docs', 'architecture.md'), 'utf8');
const releaseReadiness = fs.readFileSync(path.join(projectRoot, 'docs', 'release-readiness.md'), 'utf8');
const manualSmokeEvidence = fs.readFileSync(path.join(projectRoot, 'docs', 'manual-smoke-evidence.md'), 'utf8');
const securityThreatModel = fs.readFileSync(path.join(projectRoot, 'docs', 'security-threat-model.md'), 'utf8');
const storeSubmission = fs.readFileSync(path.join(projectRoot, 'docs', 'store-submission.md'), 'utf8');
const storeScreenshotsReadme = fs.readFileSync(path.join(projectRoot, 'docs', 'store-assets', 'screenshots', 'README.md'), 'utf8');
const migrationGuide = fs.readFileSync(path.join(projectRoot, 'docs', 'migration-guide.md'), 'utf8');
const passkeyDesign = fs.readFileSync(path.join(projectRoot, 'docs', 'passkeys-webauthn-design.md'), 'utf8');
const privacyPolicy = fs.readFileSync(path.join(projectRoot, 'docs', 'privacy-policy.md'), 'utf8');
const releaseIntegrity = fs.readFileSync(path.join(projectRoot, 'docs', 'release-integrity.md'), 'utf8');
const releaseNotesTemplate = fs.readFileSync(path.join(projectRoot, 'docs', 'release-notes-template.md'), 'utf8');
...
assert.equal(passkeyServiceSource.includes('UnsupportedResidentKeyErrorCode'), true, 'passkey backend prototype should reject unknown resident-key policy values before approval');
assert.equal(passkeyServiceSource.includes('IsKnownResidentKeyRequirement'), true, 'passkey backend prototype should centralize resident-key policy');
assert.equal(passkeyServiceSource.includes('KBB-Passkey-ResidentKey'), true, 'passkey backend prototype should preserve resident-key metadata in KeePass entries');
...
assert.equal(passkeyServiceSource.includes('HasUnsupportedExtensionNames'), true, 'passkey backend prototype should reject unsupported get assertion extensions');
...
assert.equal(passkeysProxyExperiment.includes('normalizeResidentKey'), true, 'passkey proxy experiment should normalize resident-key requests');
assert.equal(passkeysProxyExperiment.includes('requireResidentKey'), true, 'passkey proxy experiment should map legacy requireResidentKey requests');
assert.equal(passkeysProxyExperiment.includes('Passkey resident-key requirement is not supported by this build'), true, 'passkey proxy experiment should expose a WebAuthn error for unknown resident-key requirements');
...
assert.equal(passkeysProxyExperiment.includes('assertNoUnsupportedGetExtensions'), true, 'passkey proxy experiment should reject unsupported get WebAuthn extensions');
...
assert.equal(protocolModels.includes('public string ResidentKey'), true, 'protocol models should carry create resident-key preference to the backend');
...
assert.equal(protocolModels.includes('public string[] UnsupportedExtensions'), true, 'protocol models should carry unsupported requested WebAuthn extension names to the backend');
